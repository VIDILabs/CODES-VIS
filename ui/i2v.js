...
module.exports = function (require) {
    'use strict';
    var root = this, debug = true, registry = {}, $protected = {}, base = function () {
            var $objId = 101, classBase = function () {
                    $protected[$objId] = {};
                    this.objId = $objId;
                    this._protected = function () {
                        return $protected[$objId];
                    };
                    this.destroy = function () {
                        delete $protected[this.objId];
                    };
                    $objId++;
                };
            return classBase;
    ...
    function extend(classBase, classFunction) {
        function classX(option) {
            ...
            for (var key in this._super) {
                ...
                if (key[0] == '$')
                    $protected[this.objId][key] = this._super[key];
            }
            classFunction.call(this, option);
            for (var key in this) {
                if (key[0] == '$')
                    $protected[this.objId][key] = this[key];
            }
...
},{"../lib/colorbrewer/colorbrewer.js":1}],4:[function(require,module,exports){
(function (global){
...
},{"./colors":3,"./metric":5,"./svg/svg":6,"./viz":7}],5:[function(require,module,exports){
module.exports = function (require) {
    'use strict';
    ...
    return function Metric(arg) {
        var metric = map(), option = arg || {}, scale = option.scale || 'linear', domain = option.domain || [
                0,
                1
            ], margin = option.margin || 0, range = option.range || [
                0,
                1
            ];
        function map() {
        ...
        if (typeof Math.log10 === 'undefined')
            Math.log10 = function (_) {
                return Math.log(_) / Math.log(10);
            };
        metric.map = map;
        ...
        metric.domainIntervals = function (tick) {
            var intervals = [];
            if (scale == 'linear') {
                var s = Math.pow(10, Math.floor(Math.log10(Math.abs(domain[1] - domain[0]))) - 1), dist = Math.floor((domain[1] - domain[0]) / (tick * s)) * s, step = Math.abs(dist), start = Math.min(domain[0], domain[1]) + step, end = Math.max(domain[0], domain[1]);
...
module.exports = function (require) {
    ...
    return function Svg(arg) {
        'use strict;';
        var svgNS = 'http://www.w3.org/2000/svg', svg = document.createElementNS(svgNS, 'svg'), option = arg || {}, width = option.width || 400, height = option.height || 300, container = option.container || null, style = option.style || {}, padding = option.padding || {
                left: 0,
                right: 0,
                top: 0,
                bottom: 0
        ...
        function append(parent, type, attr, style) {
            ...
            obj.translate = function (x, y) {
                ...
                obj.setAttribute('transform', p + 'translate(' + [
                    x,
                    y
                ].join(',') + ') ');
                return obj;
            ...
            obj.scale = function (x, y) {
                var p = obj.getAttribute('transform') || '';
                obj.setAttribute('transform', p + 'scale(' + [
                    x,
                    y
                ].join(',') + ') ');
                return obj;
            ...
            obj.render = function (vmap) {
                var mappings = Object.keys(vmap).filter(function (k) {
                    return k != 'mark' || k != 'data';
                });
                var setFunction = {};
                mappings.forEach(function (m) {
                    if (typeof vmap[m] === 'function') {
                        ...
                        setFunction[m] = function (d) {
                            return d[vmap[m]];
                        };
                    } else {
                        setFunction[m] = function () {
                            return vmap[m];
                        };
                    }
                });
...
module.exports = function (require) {
    var Class = require('./class'), Svg = require('./svg/svg');
    return Class.create(function Viz(arg) {
        'use strict';
        var viz = this, option = arg || {}, containerId = option.container || 'body', container;
        ...
        this.$svg = function (arg) {
            var arg = arg || {}, width = arg.width || this.$width, height = arg.height || this.$height, padding = arg.padding || this.$padding;
        ...
        this.init = function () {
            ...
            this.resize(this.$width + this.$padding.left + this.$padding.right, this.$height + this.$padding.top + this.$padding.bottom);
            if (option.style)
        ...
        this.prepend = function (m) {
            if (m.tagName == 'svg')
                this.svg = [m].concat(this.svg);
            if (m.tagName == 'canvas')
                this.webgl = [m].concat(this.webgl);
        };
        this.destroy = function () {
            this._super.destroy();
